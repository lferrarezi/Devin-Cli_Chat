"use strict";var l=require("vscode"),u=require("fs"),c=require("path"),x=require("os"),D=require("child_process"),fe="devinCliChat",De=["auto","sonnet","opus","swe","gpt"],Y=["auto","adaptive","sonnet","opus","swe","gpt","codex"],ve="devinCliChat.chatHistory.v1",$e=50,Q=1024*1024,Z=50,Ne=new Set(["node_modules",".git","dist","build","out",".venv","__pycache__",".next",".nuxt",".cache","target",".idea"]),v,E,O,I,w,T=!1,F={at:0,values:void 0},z={at:0,values:void 0},H={at:0,values:void 0};function m(n){let t=new Date().toISOString();I&&I.appendLine(`[${t}] ${n}`)}function ge(){return 1e4}function Fe(){return Math.max(0,Number(p().get("cacheModelosMs")||18e5))}function q(){F={at:0,values:void 0},z={at:0,values:void 0},H={at:0,values:void 0}}function xe(n){return/^[a-zA-Z0-9][a-zA-Z0-9._-]{1,80}$/.test(String(n||"").trim())}function p(){return l.workspace.getConfiguration(fe)}function g(){return l.workspace.workspaceFolders&&l.workspace.workspaceFolders[0]?l.workspace.workspaceFolders[0].uri.fsPath:void 0}function j(){return l.workspace.workspaceFolders&&l.workspace.workspaceFolders[0]?l.workspace.workspaceFolders[0].name:"sem pasta aberta"}function be(n){if(!n)return n;let t=String(n);return t==="~"?x.homedir():t.startsWith("~/")||t.startsWith("~\\")?c.join(x.homedir(),t.slice(2)):t}function M(n){let t=be(n);return t&&(c.isAbsolute(t)?t:c.join(g()||x.homedir(),t))}function ie(n){try{return!!n&&u.existsSync(n)}catch{return!1}}function L(n){return`'${String(n).replace(/'/g,"'\\''")}'`}function $(n){let t=String(n||"").trim().toLowerCase();return!t||t==="default"||t==="padrao"||t==="padr\xE3o"?"auto":xe(t)?t:"auto"}function ye(n){return $(n)}function me(){return[...Y]}function ze(n){try{return ie(n)?JSON.parse(u.readFileSync(n,"utf8")):void 0}catch{return}}function He(){if(process.platform==="win32"){let n=process.env.APPDATA||c.join(x.homedir(),"AppData","Roaming");return c.join(n,"devin","config.json")}return c.join(x.homedir(),".config","devin","config.json")}function oe(){let n=ze(He());return n&&n.agent?ye(n.agent.model):void 0}function k(){return ye(p().get("modeloAtual")||oe()||"auto")}function we(){return $(p().get("modeloAtual")||oe()||"auto")}function V(){return String(p().get("agenteAtual")||"auto")}function G(){return String(p().get("modoExecucaoChat")||"resposta-integrada")}function P(){let n=p().get("skillsSelecionadas")||[];return Array.isArray(n)?n.map(String).filter(Boolean):[]}function b(){return String(p().get("caminhoDevin")||"devin")}function C(){return g()||x.homedir()}function ke(){return[be(p().get("gitBashPath")),process.env.GIT_BASH_PATH,"C:\\Program Files\\Git\\bin\\bash.exe","C:\\Program Files\\Git\\usr\\bin\\bash.exe","C:\\Program Files (x86)\\Git\\bin\\bash.exe",c.join(x.homedir(),"AppData","Local","Programs","Git","bin","bash.exe")].filter(Boolean).find(ie)}function je(){return process.platform==="win32"&&p().get("usarGitBashNoWindows",!0)?ke():process.env.SHELL||void 0}function U(){let n=[...(p().get("argumentosPadrao")||[]).map(String).filter(Boolean)],t=String(p().get("argumentoModelo")||"").trim(),a=we();return t&&a&&a!=="auto"&&n.push(t,a),n}function B(n){let t=p().get("prefixoPromptPadrao")||"",a=we()||k()||"auto",e=V(),i=P(),o=[`Workspace VS Code: ${j()}`,g()?`Diretorio raiz: ${g()}`:"Diretorio raiz: nao ha pasta aberta",`Modelo selecionado: ${a}`,`Agente selecionado: ${e}`,i.length?`Skills disponiveis: ${i.join(", ")}`:""].filter(Boolean).join(`
`),r=e!=="auto"?`Use o perfil/subagente Devin chamado "${e}" quando aplicavel. Se a CLI nao aceitar selecao direta de agente nesta chamada, trate este agente como persona operacional e siga as instrucoes do respectivo AGENT.md.`:"",s=i.length?`Invoque a skill via tool 'skill' quando aplicavel: ${i.map(d=>`"${d}"`).join(", ")}. Siga as instrucoes do respectivo SKILL.md.`:"";return[t,o,r,s,n].filter(Boolean).join(`

`)}function _e(n){let t=L(b()),a=U().map(L).join(" ");return n?[t,a,"-p","--",L(B(n))].filter(Boolean).join(" "):[t,a].filter(Boolean).join(" ")}function Oe(n){return n?String(n).split(/\r?\n/).filter(t=>!/were not migrated because they already exist/i.test(t)).filter(t=>!/migration.*already exist/i.test(t)).join(`
`).trim():""}function Ce(n,t,a){let e=Oe(t),i=[n||"",e||"",a&&a.message?a.message:""].join(`
`);if(/No active model set in cog manager/i.test(i))return["Modelo Devin nao configurado para esta execucao.","","A extensao tentou enviar o alias selecionado, mas o Devin CLI informou que nao ha modelo ativo no cog manager.","","Acoes recomendadas:",`1. Execute no terminal: devin model set ${k()||"auto"}`,"2. Se houver conflito de migracao de config, mantenha apenas o valor desejado em agent.model no config.json do Devin.","3. No chat, reabra o seletor de modelo e escolha um dos aliases validos: auto, sonnet, opus, swe, gpt."].join(`
`);let o=[];return n&&n.trim()&&o.push(n.trim()),e&&o.push(`STDERR:
${e}`),a&&o.push(`Falha ao executar Devin CLI: ${a.message}`),o.join(`

`)||"Sem saida do Devin CLI."}function ee(n){let t=je(),a=l.window.createTerminal({name:p().get("nomeTerminal")||"Devin Cli Chat",cwd:C(),shellPath:t,shellArgs:process.platform==="win32"&&t?["--login","-i"]:void 0});a.show(!0),a.sendText(_e(n))}function se(){if(T=!0,!w||w.killed)return!1;try{return w.kill(),!0}catch(n){return m(`cancelIntegratedRun erro: ${n&&n.message?n.message:String(n)}`),!1}}function Me(n){return new Promise(t=>{T=!1;let a=[...U(),"-p","--",B(n)];m(`runIntegrated: ${b()} ${a.slice(0,-1).join(" ")} -- [prompt ${B(n).length} chars]`),m(`  cwd: ${C()}`);let e=!1;function i(o){e||(e=!0,w=void 0,t(o))}try{w=D.execFile(b(),a,{cwd:C(),timeout:Number(p().get("timeoutChatMs")||3e5),maxBuffer:1024*1024*16,windowsHide:!0},(o,r,s)=>{if(T){i("Execucao cancelada pelo usuario.");return}if(o&&m(`runIntegrated erro: code=${o.code} signal=${o.signal} killed=${o.killed} msg=${o.message}`),s&&s.trim()&&m(`runIntegrated stderr: ${s.slice(0,500)}`),r&&r.trim()&&m(`runIntegrated stdout: ${r.slice(0,200)}...`),o&&process.platform==="win32"){X(n,o).then(i);return}i(Ce(r,s,o))}),w.on("error",o=>{if(m(`runIntegrated child error: ${o.message}`),T){i("Execucao cancelada pelo usuario.");return}process.platform==="win32"?X(n,o).then(i):i(`Falha ao iniciar Devin CLI: ${o.message}

Valide o caminho em devinCliChat.caminhoDevin e execute "Devin Cli Chat: Verificar Devin CLI".`)})}catch(o){if(m(`runIntegrated catch: ${o.message}`),T){i("Execucao cancelada pelo usuario.");return}process.platform==="win32"?X(n,o).then(i):i(`Falha ao iniciar Devin CLI: ${o.message}

Valide o caminho em devinCliChat.caminhoDevin e execute "Devin Cli Chat: Verificar Devin CLI".`)}})}function X(n,t){return new Promise(a=>{let e=ke();if(!e){a(`Falha ao executar Devin CLI: ${t.message}

Git Bash nao foi encontrado. Configure devinCliChat.gitBashPath ou ajuste devinCliChat.caminhoDevin.`);return}let i=U().map(L).join(" "),o=`${L(b())} ${i} -p -- ${L(B(n))}`;w=D.exec(o,{cwd:C(),shell:e,timeout:Number(p().get("timeoutChatMs")||3e5),maxBuffer:1024*1024*16},(r,s,d)=>{if(w=void 0,T){a("Execucao cancelada pelo usuario.");return}let h=Ce(s,d,r);a(h.replace("Falha ao executar Devin CLI:","Falha ao executar Devin CLI via Git Bash:"))})})}async function y(n,t){await p().update(n,t,l.ConfigurationTarget.Workspace),q(),ne(),v&&v.refreshMeta()}function Re(){let n=p().get("modelosDisponiveis")||[];return Array.isArray(n)?n.map(String).map(t=>t.trim()).filter(Boolean):[]}function We(){let n=p().get("arquivosCacheModelos")||[],t=Array.isArray(n)?n.map(M).filter(Boolean):[];if(process.platform==="win32"){let a=process.env.LOCALAPPDATA||c.join(x.homedir(),"AppData","Local");t.push(c.join(a,"Devin","CLI","team_settings.bin")),t.push(c.join(a,"Devin","CLI","model_configs.bin"))}else t.push(c.join(x.homedir(),".local","share","Devin","CLI","team_settings.bin")),t.push(c.join(x.homedir(),".local","share","Devin","CLI","model_configs.bin"));return Array.from(new Set(t))}function Se(n){return De.includes(String(n||"").trim().toLowerCase())}function qe(){let n=Number(p().get("limiteBytesCacheModelos")||5242880),t=[];for(let a of We())try{if(!ie(a))continue;let e=u.statSync(a);if(!e.isFile()||e.size>n)continue;let r=u.readFileSync(a).toString("utf8").match(/[a-zA-Z0-9][a-zA-Z0-9._-]{1,80}/g)||[];for(let s of r)Se(s)&&t.push(s)}catch{}return Array.from(new Set(t))}function Ee(){return new Promise(n=>{let t=String(p().get("comandoDescobertaModelos")||"").trim();if(!t){n([]);return}try{D.execFile(b(),t.split(/\s+/),{cwd:C(),timeout:Number(p().get("timeoutDescobertaModelosMs")||2500),windowsHide:!0},(a,e)=>{if(a||!e){n([]);return}try{let o=JSON.parse(e),r=Array.isArray(o)?o:o&&o.models||[];n(r.map(s=>typeof s=="string"?s:s&&(s.name||s.id)).filter(Boolean));return}catch{}let i=e.match(/[a-zA-Z0-9][a-zA-Z0-9._-]{1,80}/g)||[];n(i.filter(Se))})}catch{n([])}})}function R(){let n=Fe(),t=Date.now();if(n>0&&F.values&&t-F.at<n)return F.values;let a=[k(),oe(),...Re(),...qe(),...me()].map($).filter(Boolean),e=Array.from(new Set([...me(),...a]));return F={at:t,values:e},e}function re(){let n=Date.now();if(z.values&&n-z.at<ge())return z.values;let t=[M(p().get("diretorioAgentesWorkspace")||".devin/agents"),M(p().get("diretorioAgentesGlobal")||"~/.config/devin/agents")],a=["auto"];for(let i of t)try{if(!i||!u.existsSync(i))continue;for(let o of u.readdirSync(i)){let r=c.join(i,o,"AGENT.md");u.existsSync(r)&&a.push(o)}}catch{}let e=Array.from(new Set(a));return z={at:n,values:e},e}function le(){let n=Date.now();if(H.values&&n-H.at<ge())return H.values;let t=[M(p().get("diretorioSkillsWorkspace")||".devin/skills"),M(p().get("diretorioSkillsGlobal")||"~/.config/devin/skills"),M(".claude/skills"),M("~/.claude/skills")],a=[];for(let i of t)try{if(!i||!u.existsSync(i))continue;for(let o of u.readdirSync(i)){let r=c.join(i,o,"SKILL.md");u.existsSync(r)&&a.push(o)}}catch{}let e=Array.from(new Set(a)).sort();return H={at:n,values:e},e}function te(){let n=l.window.activeTextEditor;if(!n)return"Nenhum editor ativo.";let t=n.document,a=n.selection&&!n.selection.isEmpty?t.getText(n.selection):t.getText();return[`Arquivo: ${t.uri.fsPath}`,"Conteudo:","```",a.slice(0,6e4),"```"].join(`
`)}function he(n){return n?Math.max(1,Math.ceil(String(n).length/4)):0}function Ge(n){if(!n||!n.selection||n.selection.isEmpty)return null;let t=n.document,a=n.selection,e=t.getText(a);if(!e||!e.trim())return null;let i=t.uri.fsPath,o=c.basename(i),r=a.start.line+1,s=a.end.line+1;return{id:"sel-"+Date.now().toString(36),file:i,base:o,language:t.languageId,startLine:r,endLine:s,text:e,preview:e.split(`
`).slice(0,2).join(" ").slice(0,80),label:`${o}:${r}-${s}`}}function Ae(){try{if(p().get("usarContextoEditorAutomatico")===!1)return null;let t=String(p().get("modoContextoEditorAutomatico")||"selecao-ou-arquivo");if(t==="desativado")return null;let a=l.window.activeTextEditor;if(!a)return null;let e=a.document;if(!e||!e.uri||e.uri.scheme!=="file")return null;let i=e.uri.fsPath,o=c.basename(i),r=e.languageId||c.extname(i).slice(1)||"",s="```",d=a.selection,h=!!(d&&!d.isEmpty);if(t!=="somente-arquivo"&&h){let K=e.getText(d);if(!K||!K.trim())return null;let Le=d.start.line+1,Pe=d.end.line+1,ue=`${o}:${Le}-${Pe}`,Be=["","",`[Contexto automatico do editor: ${ue}]`,s+r,K,s].join(`
`);return{label:ue,promptBlock:Be}}if(t==="somente-selecao")return null;let f=e.getText();if(!f||!f.trim())return null;let N=Number(p().get("limiteBytesContextoEditorAutomatico")),S=Number.isFinite(N)&&N>0?N:2e5,W=Buffer.from(f,"utf8"),J=W.length>S,Ie=J?W.subarray(0,S).toString("utf8"):f,de=J?`${o} (truncado)`:o,pe=["","",`[Contexto automatico do editor: ${de}]`,s+r,Ie,s];return J&&pe.push(`[NOTA: arquivo truncado em ${S} bytes para limitar o tamanho do contexto automatico.]`),{label:de,promptBlock:pe.join(`
`)}}catch{return null}}function Te(){try{return D.execFileSync("git",["diff","--no-ext-diff"],{cwd:C(),encoding:"utf8",maxBuffer:1024*1024*8,windowsHide:!0})}catch(n){return`Nao foi possivel obter git diff: ${n.message}`}}function ne(){E||(E=l.window.createStatusBarItem(l.StatusBarAlignment.Right,90),E.command="devinCliChat.abrirPainel",E.show()),E.text=`Devin: ${k()} / ${V()}`,E.tooltip=`Workspace: ${j()} | Modo: ${G()} | Skills: ${P().length}`}async function ce(){let n=await l.window.showInputBox({title:"Modelo Devin",prompt:"Aliases aceitos pelo Devin CLI nesta build: auto, sonnet, opus, swe, gpt.",value:k()==="auto"?"":k()});if(!n||!n.trim())return;let t=$(n);t!==String(n).trim().toLowerCase()&&l.window.showInformationMessage(`Modelo "${n.trim()}" nao e aceito por esta versao do Devin CLI. Usando "${t}".`),await y("modeloAtual",t)}async function Ve(){let n=await l.window.showQuickPick([...R(),"+ Informar modelo manual"],{placeHolder:"Selecione o modelo Devin"});if(n){if(n.startsWith("+"))return ce();await y("modeloAtual",n)}}async function Ue(){let n=le();if(!n.length){l.window.showInformationMessage("Nenhuma skill encontrada em .devin/skills ou ~/.config/devin/skills.");return}let t=new Set(P()),a=n.map(i=>({label:i,picked:t.has(i)})),e=await l.window.showQuickPick(a,{canPickMany:!0,placeHolder:"Selecione skills disponiveis para o Devin"});e&&await y("skillsSelecionadas",e.map(i=>i.label))}function A(){try{return O&&O.globalState.get(ve)||[]}catch{return[]}}async function _(n){try{O&&await O.globalState.update(ve,n.slice(0,$e))}catch{}}var ae=class{constructor(t){this.context=t,this.view=void 0,this.busy=!1,this.session=this.newSession()}newSession(){return{id:"sess-"+Date.now().toString(36)+"-"+Math.random().toString(36).slice(2,6),title:"Nova conversa",createdAt:Date.now(),updatedAt:Date.now(),workspace:j(),model:k(),agent:V(),mode:G(),skills:P(),messages:[]}}async persistSession(){if(!this.session||!this.session.messages.length)return;let t=A(),a=t.findIndex(e=>e.id===this.session.id);if(this.session.updatedAt=Date.now(),!this.session.title||this.session.title==="Nova conversa"){let e=this.session.messages.find(i=>i.role==="user");e&&(this.session.title=String(e.text).slice(0,60).replace(/\s+/g," ").trim())}a>=0?t[a]=this.session:t.unshift(this.session),t.sort((e,i)=>(i.updatedAt||0)-(e.updatedAt||0)),await _(t)}resolveWebviewView(t){this.view=t,t.webview.options={enableScripts:!0,localResourceRoots:[this.context.extensionUri]},t.webview.html=this.html(t.webview),m("WebView resolvida e HTML injetado."),t.webview.onDidReceiveMessage(async a=>{try{let e=a&&a.type;if(m(`Mensagem recebida do webview: type=${e}`),e==="ready"){this.refreshMeta(),this.replaySession(),this.pushCurrentSelection();return}if(e==="clientError"){m(`ERRO no cliente webview: ${a.text||"sem detalhes"}`),this.post({type:"message",role:"assistant",text:"Falha no painel: "+(a.text||"erro sem detalhes")});return}if(e==="cancelRun"){let i=se();this.post({type:"action",ok:i,text:i?"Cancelamento solicitado.":"Nenhuma execucao integrada ativa para cancelar."});return}if(e==="verifyCli"){this.verifyCli();return}if(e==="requestSelection"){this.pushCurrentSelection(!0);return}if(e==="attachMenu"){await this.chooseAttachSource();return}if(e==="attachFiles"){await this.attachFiles();return}if(e==="pickWorkspaceFiles"){await this.pickWorkspaceFiles();return}if(e==="listWorkspace"){this.listWorkspaceDir(a.path||"");return}if(e==="attachFolder"){await this.attachFolder(a.path||"");return}if(e==="attachWorkspacePath"){await this.attachWorkspacePath(a.path||"");return}if(e==="send"){await this.send(a.text||"",{echoUser:a.echo!==!1,displayText:a.displayText||a.text||"",hasExplicitContext:!!a.hasExplicitContext});return}if(e==="terminal"){ee(a.text||""),this.post({type:"action",ok:!0,text:"Terminal aberto."});return}if(e==="setModel"){await y("modeloAtual",$(a.value||"auto"));return}if(e==="setMode"){await y("modoExecucaoChat",a.value||"resposta-integrada");return}if(e==="setAgent"){await y("agenteAtual",a.value||"auto");return}if(e==="toggleSkill"){let i=new Set(P());a.value&&i.has(a.value)?i.delete(a.value):a.value&&i.add(a.value),await y("skillsSelecionadas",Array.from(i));return}if(e==="manualModel"){await ce();return}if(e==="refreshModels"){this.refreshMeta();let i=await Ee();if(i.length){let o=Array.from(new Set([...p().get("modelosDisponiveis")||[],...i]));await p().update("modelosDisponiveis",o,l.ConfigurationTarget.Workspace)}this.refreshMeta(),this.post({type:"action",ok:!0,text:"Modelos atualizados ("+R().length+" disponiveis)."});return}if(e==="review"){await this.send("Revise o git diff atual com foco em bugs, seguranca, testes, impacto produtivo e rollback.\n\n```diff\n"+Te()+"\n```",{echoUser:!0});return}if(e==="selection"){await this.send(`Analise o contexto do editor atual.

`+te(),{echoUser:!0});return}if(e==="insertSelection"){this.post({type:"insertPrompt",text:`Analise o contexto do editor atual.

`+te()});return}if(e==="newChat"){await this.persistSession(),this.session=this.newSession(),this.post({type:"clearThread"}),this.refreshMeta();return}if(e==="getHistory"){await this.openHistory();return}if(e==="loadSession"){await this.persistSession();let o=A().find(r=>r.id===a.id);o&&(this.session=JSON.parse(JSON.stringify(o)),this.post({type:"clearThread"}),this.replaySession(),this.refreshMeta(),this.post({type:"action",ok:!0,text:"Sessao carregada: "+(o.title||o.id)}));return}if(e==="deleteSession"){let i=A().filter(o=>o.id!==a.id);await _(i),this.session&&this.session.id===a.id&&(this.session=this.newSession(),this.post({type:"clearThread"})),this.post({type:"history",sessions:i}),this.refreshMeta();return}if(e==="clearHistory"){await _([]),this.session=this.newSession(),this.post({type:"clearThread"}),this.post({type:"history",sessions:[]}),this.refreshMeta();return}this.post({type:"action",ok:!1,text:`Acao desconhecida: ${e}`})}catch(e){this.busy=!1,this.post({type:"busy",value:!1}),m(`ERRO no handler do webview: ${e&&e.message?e.message:String(e)}`),e&&e.stack&&m(e.stack),this.post({type:"message",role:"assistant",text:"Falha ao executar acao do painel: "+(e&&e.message?e.message:String(e))})}}),setTimeout(()=>this.refreshMeta(),50)}post(t){try{this.view&&this.view.webview.postMessage(t)}catch{}}async openHistory(){await this.persistSession();let t=A().filter(a=>a&&a.messages&&a.messages.length);this.post({type:"openHistory",sessions:t}),this.refreshMeta()}verifyCli(){I.show(!0),m(`Verificando Devin CLI pelo painel: ${b()}`),D.execFile(b(),["--version"],{cwd:C(),windowsHide:!0},(t,a,e)=>{if(t){let r=`Falha ao verificar Devin CLI: ${t.message}`;m(r),this.post({type:"message",role:"assistant",text:r}),this.post({type:"action",ok:!1,text:r});return}let o=`Devin CLI encontrado: ${(a||e||"ok").trim()}`;m(o),this.post({type:"message",role:"assistant",text:o}),this.post({type:"action",ok:!0,text:o})})}pushCurrentSelection(t){let a=l.window.activeTextEditor,e=Ge(a);e&&this.post({type:"selectionAvailable",selection:e})}attachmentId(t){return t+"-"+Date.now().toString(36)+Math.random().toString(36).slice(2,7)}readFileItem(t,a){let e=u.statSync(t);if(e.size>Q)return{skipped:!0,reason:`Arquivo muito grande: ${c.basename(t)} (${e.size} bytes).`};let i=u.readFileSync(t,"utf8"),o=c.extname(t).slice(1);return{id:this.attachmentId("file"),file:t,base:c.basename(t),label:a||c.basename(t),type:"file",text:i,language:o,lines:i.split(`
`).length}}readFolderItem(t,a){let e=g(),i=a||c.basename(t)||"workspace",o=[],r=[t];for(;r.length&&o.length<Z;){let s=r.pop(),d;try{d=u.readdirSync(s,{withFileTypes:!0})}catch{continue}for(let h of d){let f=c.join(s,h.name);if(h.isDirectory())!Ne.has(h.name)&&!h.name.startsWith(".")&&r.push(f);else if(h.isFile())try{if(u.statSync(f).size>Q)continue;let S=u.readFileSync(f,"utf8"),W=e&&f.startsWith(e)?c.relative(e,f):c.join(i,c.relative(t,f));if(o.push({file:f,rel:W.replace(/\\/g,"/"),base:c.basename(f),text:S,language:c.extname(f).slice(1),lines:S.split(`
`).length}),o.length>=Z)break}catch{}}}return{id:this.attachmentId("folder"),file:t,base:i,label:`${i} (${o.length})`,type:"folder",files:o,count:o.length,truncated:o.length>=Z}}async chooseAttachSource(){let t=await l.window.showQuickPick([{label:"$(folder) Pastas",description:"Anexar pasta recursivamente como chip unico",value:"folders"},{label:"$(file) Arquivos abertos",description:"Anexar arquivos atualmente abertos no editor",value:"openFiles"}],{placeHolder:"Anexar contexto ao Devin"});if(!t)return;if(t.value==="folders"){let s=await l.window.showOpenDialog({canSelectMany:!0,canSelectFiles:!1,canSelectFolders:!0,defaultUri:g()?l.Uri.file(g()):void 0,openLabel:"Anexar pasta"});if(!s||!s.length)return;let d=[];for(let h of s)try{let f=this.readFolderItem(h.fsPath,c.basename(h.fsPath));f.files&&f.files.length&&d.push(f)}catch(f){this.post({type:"action",ok:!1,text:`Falha ao anexar pasta ${h.fsPath}: ${f.message}`})}d.length&&this.post({type:"attachItems",items:d}),this.post({type:"action",ok:!0,text:`Anexadas ${d.length} pasta(s).`});return}let a=l.workspace.textDocuments.filter(s=>s.uri&&s.uri.scheme==="file"&&!s.isUntitled);if(!a.length){this.post({type:"action",ok:!1,text:"Nenhum arquivo aberto para anexar."});return}let e=g()||"",i=a.map(s=>({label:"$(file) "+c.basename(s.uri.fsPath),description:e?c.dirname(c.relative(e,s.uri.fsPath)):c.dirname(s.uri.fsPath),detail:s.uri.fsPath,doc:s})),o=await l.window.showQuickPick(i,{canPickMany:!0,placeHolder:"Selecione arquivos abertos para anexar"});if(!o||!o.length)return;let r=[];for(let s of o){let d=s.doc,h=d.getText();if(Buffer.byteLength(h,"utf8")>Q){this.post({type:"action",ok:!1,text:`Arquivo muito grande: ${c.basename(d.uri.fsPath)}.`});continue}r.push({id:this.attachmentId("file"),file:d.uri.fsPath,base:c.basename(d.uri.fsPath),label:c.basename(d.uri.fsPath),type:"file",text:h,language:d.languageId||c.extname(d.uri.fsPath).slice(1),lines:h.split(`
`).length})}r.length&&this.post({type:"attachItems",items:r})}async attachFiles(){try{let t=await l.window.showOpenDialog({canSelectMany:!0,canSelectFiles:!0,canSelectFolders:!1,defaultUri:g()?l.Uri.file(g()):void 0,openLabel:"Anexar ao chat"});if(!t||!t.length)return;let a=[];for(let e of t)try{let i=u.statSync(e.fsPath);if(i.size>1024*1024){a.push({id:"file-"+Date.now().toString(36)+Math.random().toString(36).slice(2,5),file:e.fsPath,base:c.basename(e.fsPath),label:c.basename(e.fsPath),type:"file",text:`Arquivo ${e.fsPath} muito grande (${i.size} bytes) - nao anexado.`,language:"",tooBig:!0});continue}let o=u.readFileSync(e.fsPath,"utf8"),r=c.extname(e.fsPath).slice(1);a.push({id:"file-"+Date.now().toString(36)+Math.random().toString(36).slice(2,5),file:e.fsPath,base:c.basename(e.fsPath),label:c.basename(e.fsPath),type:"file",text:o,language:r,lines:o.split(`
`).length})}catch(i){this.post({type:"action",ok:!1,text:`Falha ao ler ${e.fsPath}: ${i.message}`})}a.length&&this.post({type:"attachItems",items:a})}catch(t){this.post({type:"action",ok:!1,text:"Falha ao anexar: "+(t&&t.message?t.message:String(t))})}}async pickWorkspaceFiles(){try{let t=await l.workspace.findFiles("**/*","**/{node_modules,.git,dist,build,out,.venv,__pycache__}/**",5e3);if(!t.length){this.post({type:"action",ok:!1,text:"Nenhum arquivo encontrado."});return}let a=g()||"",e=t.map(r=>({label:c.relative(a,r.fsPath)||c.basename(r.fsPath),description:"",uri:r})),i=await l.window.showQuickPick(e,{placeHolder:"Selecione arquivos do workspace para anexar",canPickMany:!0,matchOnDescription:!0});if(!i||!i.length)return;let o=[];for(let r of i)try{if(u.statSync(r.uri.fsPath).size>1024*1024)continue;let d=u.readFileSync(r.uri.fsPath,"utf8"),h=c.extname(r.uri.fsPath).slice(1);o.push({id:"file-"+Date.now().toString(36)+Math.random().toString(36).slice(2,5),file:r.uri.fsPath,base:c.basename(r.uri.fsPath),label:c.basename(r.uri.fsPath),type:"file",text:d,language:h})}catch{}o.length&&this.post({type:"attachItems",items:o})}catch(t){this.post({type:"action",ok:!1,text:"Falha: "+(t&&t.message?t.message:String(t))})}}listWorkspaceDir(t){try{let a=g();if(!a){this.post({type:"workspaceList",path:t||"",entries:[],error:"Sem workspace aberto."});return}let e=String(t||"").replace(/\\/g,"/").replace(/^\/+/,"").replace(/\.\.\/?/g,""),i=e?c.join(a,e):a;if(!u.existsSync(i)||!u.statSync(i).isDirectory()){this.post({type:"workspaceList",path:"",entries:[],error:"Diretorio invalido."});return}let o=new Set(["node_modules",".git","dist","build","out",".venv","__pycache__",".next",".nuxt",".cache","target",".idea"]),r=u.readdirSync(i,{withFileTypes:!0}).filter(s=>!s.name.startsWith(".")||[".cognition",".devin",".claude",".cursor",".vscode"].includes(s.name)).filter(s=>!(s.isDirectory()&&o.has(s.name))).map(s=>{let d=0;try{s.isFile()&&(d=u.statSync(c.join(i,s.name)).size)}catch{}return{name:s.name,isDir:s.isDirectory(),size:d}}).sort((s,d)=>d.isDir-s.isDir||s.name.localeCompare(d.name));this.post({type:"workspaceList",path:e,entries:r})}catch(a){this.post({type:"workspaceList",path:"",entries:[],error:a.message})}}async attachWorkspacePath(t){let a=g();if(!a)return;let e=c.join(a,String(t||"").replace(/\\/g,"/").replace(/^\/+/,""));if(!u.existsSync(e)){this.post({type:"action",ok:!1,text:"Caminho invalido."});return}if(u.statSync(e).isDirectory()){await this.attachFolder(t);return}try{let i=u.statSync(e);if(i.size>1024*1024){this.post({type:"action",ok:!1,text:`Arquivo muito grande: ${c.basename(e)} (${i.size} bytes).`});return}let o=u.readFileSync(e,"utf8"),r=c.extname(e).slice(1);this.post({type:"attachItems",items:[{id:"file-"+Date.now().toString(36)+Math.random().toString(36).slice(2,5),file:e,base:c.basename(e),label:c.basename(e),type:"file",text:o,language:r}]})}catch(i){this.post({type:"action",ok:!1,text:"Falha: "+i.message})}}async attachFolder(t){let a=g();if(!a)return;let e=String(t||"").replace(/\\/g,"/").replace(/^\/+/,"").replace(/\.\.\/?/g,""),i=e?c.join(a,e):a;if(!u.existsSync(i)||!u.statSync(i).isDirectory()){this.post({type:"action",ok:!1,text:"Pasta invalida."});return}let o=this.readFolderItem(i,e?c.basename(i):j());o.files&&o.files.length&&this.post({type:"attachItems",items:[o]}),this.post({type:"action",ok:!0,text:`Pasta anexada como chip unico: ${o.label}.`})}replaySession(){if(!(!this.session||!this.session.messages.length))for(let t of this.session.messages)this.post({type:"message",role:t.role,text:t.text,replay:!0})}refreshMeta(){let t={type:"meta",models:Y,model:k(),agents:["auto"],agent:V(),skills:[],selectedSkills:P(),mode:G(),workspace:j(),sessionId:this.session&&this.session.id,sessionTitle:this.session&&this.session.title,modelLocked:!1,hasMessages:!!(this.session&&this.session.messages&&this.session.messages.length),tokensTotal:this.session&&this.session.tokens||0,tokensIn:this.session&&this.session.tokensIn||0,tokensOut:this.session&&this.session.tokensOut||0,modelStatus:"modelo: auto"};try{t.models=R()}catch{t.models=Y}try{t.agents=re()}catch{t.agents=["auto"]}try{t.skills=le()}catch{t.skills=[]}try{t.recentSessions=A().slice(0,3).map(a=>({id:a.id,title:a.title||"Sem titulo",updatedAt:a.updatedAt,messages:(a.messages||[]).length,model:a.model||"auto"}))}catch{t.recentSessions=[]}try{t.modelStatus=`${t.models.length} modelos | ${t.skills.length} skills`}catch{}this.post(t)}async send(t,a){let e=String(t||"").trim();if(!e)return;let i=String(a&&a.displayText?a.displayText:e).trim(),o=null;if(!a||!a.hasExplicitContext){let s=Ae();s&&s.promptBlock&&(e=e+s.promptBlock,i=i+`

[Contexto automatico: `+s.label+"]",o=s.label)}if(this.busy){this.post({type:"message",role:"assistant",text:"Ja existe uma execucao em andamento. A concorrencia permanece controlada no backend."});return}this.busy=!0;let r=he(B(e));this.session.tokensIn=(this.session.tokensIn||0)+r,this.session.tokens=(this.session.tokens||0)+r,(!a||a.echoUser!==!1)&&this.post({type:"message",role:"user",text:i}),this.session.messages.push({role:"user",text:i,fullText:e,ts:Date.now(),tokens:r}),this.post({type:"busy",value:!0}),o&&this.post({type:"ctxHint",text:"\u{1F4C4} Contexto automatico: "+o}),this.post({type:"action",ok:!0,text:"Enviando para o Devin CLI..."}),this.refreshMeta();try{let s=G();if(m(`send: modo=${s} prompt=${e.length} chars`),s==="terminal"){ee(e);let f="Sessao aberta no terminal integrado, ja posicionada na pasta aberta no VS Code.";this.post({type:"message",role:"assistant",text:f}),this.session.messages.push({role:"assistant",text:f,ts:Date.now()});return}let d=await Me(e);m(`send: resposta recebida (${d?d.length:0} chars)`);let h=he(d);this.session.tokensOut=(this.session.tokensOut||0)+h,this.session.tokens=(this.session.tokens||0)+h,this.post({type:"message",role:"assistant",text:d}),this.session.messages.push({role:"assistant",text:d,ts:Date.now(),tokens:h})}catch(s){m(`send ERRO: ${s&&s.message?s.message:String(s)}`);let d="Falha ao enviar para o Devin CLI: "+(s&&s.message?s.message:String(s));this.post({type:"message",role:"assistant",text:d}),this.session.messages.push({role:"assistant",text:d,ts:Date.now()})}finally{this.busy=!1,this.post({type:"busy",value:!1}),await this.persistSession(),this.refreshMeta()}}html(t){let a=Date.now().toString(36),e={history:'<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6"/><path d="M8 4.5V8l2.4 1.6"/></svg>',plus:'<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"><path d="M8 3v10M3 8h10"/></svg>',refresh:'<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M13 8a5 5 0 1 1-1.5-3.55"/><path d="M13 3v3h-3"/></svg>',terminal:'<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="12" height="10" rx="1.5"/><path d="M5 7l2 1.5L5 10M8.5 10.5h3"/></svg>',lock:'<svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3.5" y="7" width="9" height="6.5" rx="1"/><path d="M5.5 7V5a2.5 2.5 0 0 1 5 0v2"/></svg>',paperclip:'<svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.5 6.5L6.8 11.2a2 2 0 0 1-2.8-2.8l5.4-5.4a3 3 0 0 1 4.2 4.2l-5.4 5.4a4 4 0 0 1-5.7-5.7L7.5 2.5"/></svg>',attach:'<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11.5 6.5L6.8 11.2a2 2 0 0 1-2.8-2.8l5.4-5.4a3 3 0 0 1 4.2 4.2l-5.4 5.4a4 4 0 0 1-5.7-5.7L7.5 2.5"/></svg>',file:'<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 1.5h6.5L13 5v9.5H3z"/><path d="M9.5 1.5V5H13"/></svg>',folder:'<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2 4.5a1 1 0 0 1 1-1h3l1.5 1.5h5.5a1 1 0 0 1 1 1V12a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1z"/></svg>',close:'<svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M4 4l8 8M12 4l-8 8"/></svg>',send:'<svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M2 13.5L14 8 2 2.5 2 7l8 1-8 1z"/></svg>',brain:'<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><path d="M5.5 3a2 2 0 0 0-2 2 2 2 0 0 0-1 3.5 2 2 0 0 0 1.5 3 2 2 0 0 0 3.5 0V3.5A1.5 1.5 0 0 0 5.5 3z"/><path d="M10.5 3a2 2 0 0 1 2 2 2 2 0 0 1 1 3.5 2 2 0 0 1-1.5 3 2 2 0 0 1-3.5 0V3.5A1.5 1.5 0 0 1 10.5 3z"/></svg>',bot:'<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5.5" width="10" height="7" rx="1.5"/><path d="M8 3v2.5M5.5 8.5h.01M10.5 8.5h.01M2 9.5v1.5M14 9.5v1.5"/></svg>',mode:'<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><path d="M2 5.5h12M2 8h8M2 10.5h10"/></svg>',sparkle:'<svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M8 2l1.2 3.4L12.5 6.5 9.2 7.6 8 11l-1.2-3.4L3.5 6.5 6.8 5.4z"/></svg>',caret:'<svg width="10" height="10" viewBox="0 0 16 16" fill="currentColor"><path d="M5 3l6 5-6 5z"/></svg>'},i=[{label:"auto",value:"auto"},{label:"sonnet",value:"sonnet"},{label:"opus",value:"opus"},{label:"swe",value:"swe"},{label:"gpt",value:"gpt"}];return`<!doctype html><html lang="pt-BR"><head><meta charset="UTF-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src ${t.cspSource} data:; style-src 'unsafe-inline' ${t.cspSource}; script-src 'nonce-${a}';"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>
...
var MODEL_TREE = ${JSON.stringify(i)};
...
</script></body></html>`}};async function Je(n){O=n,I=l.window.createOutputChannel("Devin Cli Chat"),n.subscriptions.push(I),m(`Extens\xE3o ativando \u2014 VS Code ${l.version}, extens\xE3o ${n.extension.packageJSON.version}`),m(`Plataforma: ${process.platform} ${process.arch}`),m(`Devin CLI path configurado: ${b()}`),m(`Workspace: ${g()||"nenhum"}`),v=new ae(n),n.subscriptions.push(l.window.registerWebviewViewProvider("devinCliChat.chatView",v,{webviewOptions:{retainContextWhenHidden:!0}})),n.subscriptions.push(l.commands.registerCommand("devinCliChat.abrirPainel",async()=>l.commands.executeCommand("workbench.view.extension.devinCliChat"))),n.subscriptions.push(l.commands.registerCommand("devinCliChat.abrirHistorico",async()=>{await l.commands.executeCommand("workbench.view.extension.devinCliChat"),setTimeout(()=>{v&&v.openHistory()},100)})),n.subscriptions.push(l.commands.registerCommand("devinCliChat.novaSessao",()=>ee(""))),n.subscriptions.push(l.commands.registerCommand("devinCliChat.revisarDiff",async()=>{await l.commands.executeCommand("workbench.view.extension.devinCliChat"),v&&await v.send("Revise o git diff atual com foco em bugs, seguranca, testes, impacto produtivo e rollback.\n\n```diff\n"+Te()+"\n```")})),n.subscriptions.push(l.commands.registerCommand("devinCliChat.enviarSelecao",async()=>{await l.commands.executeCommand("workbench.view.extension.devinCliChat"),v&&await v.send(`Analise o contexto do editor atual.

`+te())})),n.subscriptions.push(l.commands.registerCommand("devinCliChat.selecionarModelo",Ve)),n.subscriptions.push(l.commands.registerCommand("devinCliChat.definirModeloManual",ce)),n.subscriptions.push(l.commands.registerCommand("devinCliChat.atualizarModelos",async()=>{q(),v&&v.refreshMeta();let e=await Ee();if(e.length){let i=Array.from(new Set([...p().get("modelosDisponiveis")||[],...e]));await p().update("modelosDisponiveis",i,l.ConfigurationTarget.Workspace),q()}v&&v.refreshMeta(),l.window.showInformationMessage(`Modelos atualizados (${R().length} disponiveis).`)})),n.subscriptions.push(l.commands.registerCommand("devinCliChat.selecionarAgente",async()=>{let e=await l.window.showQuickPick(re(),{placeHolder:"Selecione o agente Devin"});e&&await y("agenteAtual",e)})),n.subscriptions.push(l.commands.registerCommand("devinCliChat.selecionarSkills",Ue)),n.subscriptions.push(l.commands.registerCommand("devinCliChat.selecionarModo",async()=>{let e=await l.window.showQuickPick([{label:"Integrado (resposta no chat)",value:"resposta-integrada"},{label:"Terminal",value:"terminal"}],{placeHolder:"Selecione o modo de execucao"});e&&await y("modoExecucaoChat",e.value)})),n.subscriptions.push(l.commands.registerCommand("devinCliChat.limparHistorico",async()=>{await l.window.showWarningMessage("Limpar todo o historico de chats?",{modal:!0},"Limpar")==="Limpar"&&(await _([]),v&&v.post({type:"history",sessions:[]}))})),n.subscriptions.push(l.commands.registerCommand("devinCliChat.verificarCli",()=>{I.show(!0),m(`Verificando Devin CLI: ${b()}`),D.execFile(b(),["--version"],{cwd:C(),windowsHide:!0},(e,i,o)=>{if(e)m(`verificarCli ERRO: code=${e.code} msg=${e.message}`),l.window.showErrorMessage(`Falha ao verificar Devin CLI: ${e.message}`);else{let r=(i||o||"ok").trim();m(`verificarCli OK: ${r}`),l.window.showInformationMessage(`Devin CLI encontrado: ${r}`)}})}));let t,a=()=>{t&&clearTimeout(t),t=setTimeout(()=>{v&&v.pushCurrentSelection(!0)},150)};n.subscriptions.push(l.window.onDidChangeTextEditorSelection(a)),n.subscriptions.push(l.window.onDidChangeActiveTextEditor(a)),n.subscriptions.push(l.workspace.onDidChangeConfiguration(e=>{e.affectsConfiguration(fe)&&(q(),ne(),v&&v.refreshMeta())})),ne()}function Ke(){se()}module.exports={activate:Je,deactivate:Ke,_internal:{baseArgs:U,fullPrompt:B,runIntegrated:Me,modelsForUi:R,scanAgents:re,scanSkills:le,loadHistory:A,saveHistory:_,sanitizeModel:$,isSafeModelId:xe,cancelIntegratedRun:se,automaticEditorContext:Ae}};
