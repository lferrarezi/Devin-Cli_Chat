"use strict";var d=require("vscode"),u=require("fs"),l=require("path"),y=require("os"),F=require("child_process"),Ke=require("crypto"),Se="devinCliChat",Ze=["auto","sonnet","opus","swe","gpt"],ae=["auto","adaptive","sonnet","opus","swe","gpt","codex"],Me="devinCliChat.chatHistory.v1",Qe=50,ee=1024*1024,te=50,Xe=new Set(["node_modules",".git","dist","build","out",".venv","__pycache__",".next",".nuxt",".cache","target",".idea"]),g,L,W,P,G=new Map,H={at:0,values:void 0},_={at:0,values:void 0},j={at:0,values:void 0};function f(n){let e=new Date().toISOString();P&&P.appendLine(`[${e}] ${n}`)}function Ee(){return 1e4}function Ye(){return Math.max(0,Number(m().get("cacheModelosMs")||18e5))}function O(){H={at:0,values:void 0},_={at:0,values:void 0},j={at:0,values:void 0}}function Ae(n){return/^[a-zA-Z0-9][a-zA-Z0-9._-]{1,80}$/.test(String(n||"").trim())}function m(){return d.workspace.getConfiguration(Se)}function v(){return d.workspace.workspaceFolders&&d.workspace.workspaceFolders[0]?d.workspace.workspaceFolders[0].uri.fsPath:void 0}function B(){return d.workspace.workspaceFolders&&d.workspace.workspaceFolders[0]?d.workspace.workspaceFolders[0].name:"sem pasta aberta"}function Ie(n){if(!n)return n;let e=String(n);return e==="~"?y.homedir():e.startsWith("~/")||e.startsWith("~\\")?l.join(y.homedir(),e.slice(2)):e}function C(n){let e=Ie(n);return e&&(l.isAbsolute(e)?e:l.join(v()||y.homedir(),e))}function ce(n){try{return!!n&&u.existsSync(n)}catch{return!1}}function Te(){return Ke.randomBytes(18).toString("base64")}function E(n,e){if(typeof n!="string")return;let a=e||2e4;return n.length>a?n.slice(0,a):n}function ye(n){return typeof n=="boolean"?n:void 0}function et(n){return typeof n=="string"&&/^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,159}$/.test(n)}function Le(n){if(!n||typeof n!="object"||Array.isArray(n))return null;let e=E(n.type,40);if(new Set(["ready","cancelRun","verifyCli","requestSelection","attachMenu","attachFiles","pickWorkspaceFiles","manualModel","refreshModels","review","selection","insertSelection","newChat","getHistory","clearHistory","importSkillFile"]).has(e))return{type:e};if(e==="clientError")return{type:e,text:E(n.text,2e3)||""};if(e==="send"){let t=E(n.text,2e5);if(t===void 0)return null;let o={type:e,text:t},i=E(n.displayText,2e5),s=ye(n.echo),r=ye(n.hasExplicitContext);return i!==void 0&&(o.displayText=i),s!==void 0&&(o.echo=s),r!==void 0&&(o.hasExplicitContext=r),o}if(e==="terminal"){let t=E(n.text,2e5);return t===void 0?null:{type:e,text:t}}if(e==="setModel"||e==="setMode"||e==="setAgent"||e==="toggleSkill"){let t=E(n.value,200);return t===void 0?null:{type:e,value:t}}if(e==="listWorkspace"||e==="attachFolder"||e==="attachWorkspacePath"){let t=E(n.path||"",2e3);return t===void 0?null:{type:e,path:t}}return(e==="loadSession"||e==="deleteSession"||e==="exportSession")&&et(n.id)?{type:e,id:n.id}:null}function Pe(n){let e=String(n||"").trim();if(!e.startsWith("/"))return null;let t=(e.slice(1).split(/\s+/).shift()||"").toLowerCase(),i=e.slice(t.length+2).trim()||"o contexto atual",r={review:"Revise o git diff atual com foco em bugs, seguranca, testes, impacto produtivo e rollback.",tests:`Proponha e implemente testes para ${i}. Priorize cobertura de comportamento, regressao e caminhos de erro.`,plan:`Crie um plano tecnico objetivo para ${i}. Agrupe por impacto, risco e ordem de execucao.`,explain:`Explique ${i} com foco em arquitetura, fluxo de dados e pontos de manutencao.`,security:`Revise ${i} com foco em riscos de seguranca, entrada nao confiavel, execucao de comandos e exposicao de dados.`,docs:`Gere documentacao pratica para ${i}, incluindo objetivo, uso, limites e exemplos.`,"commit-msg":"Gere uma mensagem de commit convencional para as mudancas atuais, com escopo claro e resumo curto."}[t];return r?{command:t,text:r,displayText:e}:null}function z(n){return String(n??"").replace(/\r?\n/g," ").trim()}function Be(n){let e=n||{},a=[];a.push("# "+(z(e.title)||"Sessao Devin")),a.push(""),a.push("- Workspace: "+(z(e.workspace)||B())),a.push("- Modelo: "+(z(e.model)||"auto")),a.push("- Agente: "+(z(e.agent)||"auto")),a.push("- Modo: "+(z(e.mode)||"resposta-integrada")),e.createdAt&&a.push("- Criada em: "+new Date(e.createdAt).toISOString()),e.updatedAt&&a.push("- Atualizada em: "+new Date(e.updatedAt).toISOString()),a.push("");let t=Array.isArray(e.messages)?e.messages:[];for(let o of t){let i=String(o&&o.role||"message").toLowerCase();a.push("## "+i.charAt(0).toUpperCase()+i.slice(1)),o&&o.ts&&a.push("_"+new Date(o.ts).toISOString()+"_"),a.push(""),a.push(String(o&&o.text||"").trim()),a.push("")}return a.join(`
`).trim()+`
`}function D(n){return`'${String(n).replace(/'/g,"'\\''")}'`}function N(n){let e=String(n||"").trim().toLowerCase();return!e||e==="default"||e==="padrao"||e==="padr\xE3o"?"auto":Ae(e)?e:"auto"}function De(n){return N(n)}function we(){return[...ae]}function tt(n){try{return ce(n)?JSON.parse(u.readFileSync(n,"utf8")):void 0}catch{return}}function nt(){if(process.platform==="win32"){let n=process.env.APPDATA||l.join(y.homedir(),"AppData","Roaming");return l.join(n,"devin","config.json")}return l.join(y.homedir(),".config","devin","config.json")}function de(){let n=tt(nt());return n&&n.agent?De(n.agent.model):void 0}function S(){let n=m().get("modeloAtual");return De(n??(de()||"auto"))}function $e(){let n=m().get("modeloAtual");return N(n??(de()||"auto"))}function Z(){return String(m().get("agenteAtual")||"auto")}function J(){return String(m().get("modoExecucaoChat")||"resposta-integrada")}function I(){let n=m().get("skillsSelecionadas")||[];return Array.isArray(n)?n.map(String).filter(Boolean):[]}function w(){return String(m().get("caminhoDevin")||"devin")}function M(){return v()||y.homedir()}function ke(n){try{if(u.existsSync(n))return u.realpathSync.native?u.realpathSync.native(n):u.realpathSync(n)}catch{}return l.resolve(n)}function at(n,e){let a=l.relative(n,e);return a===""||!!a&&!a.startsWith("..")&&!l.isAbsolute(a)}function K(n){let e=v();if(!e)return null;let a=ke(e),t=String(n||"").replace(/\\/g,l.sep).trim(),o=l.isAbsolute(t)?l.resolve(t):l.resolve(a,t),i=ke(o);return at(a,i)?i:null}function Fe(){return[Ie(m().get("gitBashPath")),process.env.GIT_BASH_PATH,"C:\\Program Files\\Git\\bin\\bash.exe","C:\\Program Files\\Git\\usr\\bin\\bash.exe","C:\\Program Files (x86)\\Git\\bin\\bash.exe",l.join(y.homedir(),"AppData","Local","Programs","Git","bin","bash.exe")].filter(Boolean).find(ce)}function ot(){return process.platform==="win32"&&m().get("usarGitBashNoWindows",!0)?Fe():process.env.SHELL||void 0}function Q(){let n=[...(m().get("argumentosPadrao")||[]).map(String).filter(Boolean)],e=String(m().get("argumentoModelo")||"").trim(),a=$e();return e&&a&&a!=="auto"&&n.push(e,a),n}function $(n){let e=m().get("prefixoPromptPadrao")||"",a=$e()||S()||"auto",t=Z(),o=I(),i=[`Workspace VS Code: ${B()}`,v()?`Diretorio raiz: ${v()}`:"Diretorio raiz: nao ha pasta aberta",`Modelo selecionado: ${a}`,`Agente selecionado: ${t}`,o.length?`Skills disponiveis: ${o.join(", ")}`:""].filter(Boolean).join(`
`),s=t!=="auto"?`Use o perfil/subagente Devin chamado "${t}" quando aplicavel. Se a CLI nao aceitar selecao direta de agente nesta chamada, trate este agente como persona operacional e siga as instrucoes do respectivo AGENT.md.`:"",r=o.length?`Invoque a skill via tool 'skill' quando aplicavel: ${o.map(c=>`"${c}"`).join(", ")}. Siga as instrucoes do respectivo SKILL.md.`:"";return[e,i,s,r,n].filter(Boolean).join(`

`)}function it(n){let e=D(w()),a=Q().map(D).join(" ");return n?[e,a,"-p","--",D($(n))].filter(Boolean).join(" "):[e,a].filter(Boolean).join(" ")}function st(n){return n?String(n).split(/\r?\n/).filter(e=>!/were not migrated because they already exist/i.test(e)).filter(e=>!/migration.*already exist/i.test(e)).join(`
`).trim():""}function Ne(n,e,a){let t=st(e),o=[n||"",t||"",a&&a.message?a.message:""].join(`
...
</script></body></html>`}};async function ft(n){W=n,P=d.window.createOutputChannel("Devin Cli Chat"),n.subscriptions.push(P),f(`Extens\xE3o ativando \u2014 VS Code ${d.version}, extens\xE3o ${n.extension.packageJSON.version}`),f(`Plataforma: ${process.platform} ${process.arch}`),f(`Devin CLI path configurado: ${w()}`),f(`Workspace: ${v()||"nenhum"}`),g=new le(n),n.subscriptions.push(d.window.registerWebviewViewProvider("devinCliChat.chatView",g,{webviewOptions:{retainContextWhenHidden:!0}})),n.subscriptions.push(d.commands.registerCommand("devinCliChat.abrirPainel",async()=>d.commands.executeCommand("workbench.view.extension.devinCliChat"))),n.subscriptions.push(d.commands.registerCommand("devinCliChat.abrirHistorico",async()=>{await d.commands.executeCommand("workbench.view.extension.devinCliChat"),setTimeout(()=>{g&&g.openHistory()},100)})),n.subscriptions.push(d.commands.registerCommand("devinCliChat.exportarSessaoAtual",async()=>{await d.commands.executeCommand("workbench.view.extension.devinCliChat"),g&&await g.exportSession()})),n.subscriptions.push(d.commands.registerCommand("devinCliChat.novaSessao",()=>oe(""))),n.subscriptions.push(d.commands.registerCommand("devinCliChat.revisarDiff",async()=>{await d.commands.executeCommand("workbench.view.extension.devinCliChat"),g&&await g.send("Revise o git diff atual com foco em bugs, seguranca, testes, impacto produtivo e rollback.\n\n```diff\n"+se()+"\n```")})),n.subscriptions.push(d.commands.registerCommand("devinCliChat.enviarSelecao",async()=>{await d.commands.executeCommand("workbench.view.extension.devinCliChat"),g&&await g.send(`Analise o contexto do editor atual.

`+ie())})),n.subscriptions.push(d.commands.registerCommand("devinCliChat.selecionarModelo",mt)),n.subscriptions.push(d.commands.registerCommand("devinCliChat.definirModeloManual",ge)),n.subscriptions.push(d.commands.registerCommand("devinCliChat.atualizarModelos",async()=>{O(),g&&g.refreshMeta();let t=await je();if(t.length){let o=Array.from(new Set([...m().get("modelosDisponiveis")||[],...t]));await m().update("modelosDisponiveis",o,d.ConfigurationTarget.Workspace),O()}g&&g.refreshMeta(),d.window.showInformationMessage(`Modelos atualizados (${V().length} disponiveis).`)})),n.subscriptions.push(d.commands.registerCommand("devinCliChat.selecionarAgente",async()=>{let t=await d.window.showQuickPick(he(),{placeHolder:"Selecione o agente Devin"});t&&await b("agenteAtual",t)})),n.subscriptions.push(d.commands.registerCommand("devinCliChat.selecionarSkills",ht)),n.subscriptions.push(d.commands.registerCommand("devinCliChat.selecionarModo",async()=>{let t=await d.window.showQuickPick([{label:"Integrado (resposta no chat)",value:"resposta-integrada"},{label:"Terminal",value:"terminal"}],{placeHolder:"Selecione o modo de execucao"});t&&await b("modoExecucaoChat",t.value)})),n.subscriptions.push(d.commands.registerCommand("devinCliChat.limparHistorico",async()=>{await d.window.showWarningMessage("Limpar todo o historico de chats?",{modal:!0},"Limpar")==="Limpar"&&(await q([]),g&&g.post({type:"history",sessions:[]}))})),n.subscriptions.push(d.commands.registerCommand("devinCliChat.verificarCli",()=>{P.show(!0),f(`Verificando Devin CLI: ${w()}`),F.execFile(w(),["--version"],{cwd:M(),windowsHide:!0},(t,o,i)=>{if(t)f(`verificarCli ERRO: code=${t.code} msg=${t.message}`),d.window.showErrorMessage(`Falha ao verificar Devin CLI: ${t.message}`);else{let s=(o||i||"ok").trim();f(`verificarCli OK: ${s}`),d.window.showInformationMessage(`Devin CLI encontrado: ${s}`)}})}));let e,a=()=>{e&&clearTimeout(e),e=setTimeout(()=>{g&&g.pushCurrentSelection(!0)},150)};n.subscriptions.push(d.window.onDidChangeTextEditorSelection(a)),n.subscriptions.push(d.window.onDidChangeActiveTextEditor(a)),n.subscriptions.push(d.workspace.onDidChangeConfiguration(t=>{t.affectsConfiguration(Se)&&(O(),re(),g&&g.refreshMeta())})),re()}function gt(){for(let n of ze())me(n)}module.exports={activate:ft,deactivate:gt,_internal:{baseArgs:Q,fullPrompt:$,runIntegrated:He,modelsForUi:V,scanAgents:he,scanSkills:fe,skillNameFromMarkdownFile:Oe,importSkillMarkdownFile:Re,loadHistory:A,saveHistory:q,sanitizeModel:N,isSafeModelId:Ae,cancelIntegratedRun:me,automaticEditorContext:qe,resolveWorkspacePathSafe:K,registerRunState:pe,unregisterRunState:ue,activeRunIds:ze,createNonce:Te,validateWebviewMessage:Le,expandSlashCommand:Pe,exportSessionMarkdown:Be}};
