...
function scanAgents() {
  const now = Date.now();
  if (agentsCache.values && now - agentsCache.at < metadataCacheMs()) return agentsCache.values;
  const dirs = [
...
function automaticEditorContext() {
  try {
    ...
    const doc = editor.document;
    if (!doc || !doc.uri || doc.uri.scheme !== 'file') return null;
    const file = doc.uri.fsPath;
    const base = path.basename(file);
    ...
    if (mode === 'somente-selecao') return null;
    ...
    if (truncated) lines.push(`[NOTA: arquivo truncado em ${maxBytes} bytes para limitar o tamanho do contexto automatico.]`);
...
class ChatViewProvider {
  ...
  resolveWebviewView(view) {
    ...
    view.webview.onDidReceiveMessage(async (rawMessage) => {
      try {
        ...
        if (type === 'verifyCli') { this.verifyCli(); return; }
  ...
  async send(text, options) {
    ...
    if (autoCtxLabel) this.post({ type: 'ctxHint', text: '📄 Contexto automatico: ' + autoCtxLabel });
